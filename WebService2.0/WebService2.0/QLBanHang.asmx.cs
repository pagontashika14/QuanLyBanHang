    ...
    public class QLBanHang : System.Web.Services.WebService
    {
        ...
        public void TestService(int i)
        {
            try
            {
                ...
                {
                    var hangHoa = context.DM_HANG_HOA.Where(s => s.ID == 1).FirstOrDefault();
                    var hang = new HangHoa(hangHoa,context);
                    object data;
                    data = i == 0 ? hang.GetDonGian() : hang;
        ...
        public void TimKiemHangHoa()
        {
            try
            {
                ...
                {
                    data = Function.TimKiemHangHoaTheoTag(list_id_loai_tag);
                }
                var result = new KetQuaTraVe(true, "Thành công", data);
                TraKetQua(result);
            }
